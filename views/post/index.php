            ...
            <div class="blog-post-area-style">
                ...
                <?php foreach($posts as $post): ?>
                    ...
                    <div class="single-post">
                        <?= \yii\helpers\Html::img("@web/{$post->img}") ;?> <!-- вывод избражения из корневой папки web -->
                        <h3><a href="<?= \yii\helpers\Url::to(['post/view', 'id' => $post->id]) ?>"><?=$post->title;?></a></h3>
                        <h4>
                            <span>Category: <span class="author-name">
                                <a href="<?= \yii\helpers\Url::to(['category/view', 'alias' => $post->category->alias]) ;?>"><?= $post->category->title ;?></a>
                            </span></span>
                        </h4>
                        <?= $post->excerpt ;?>
                        <h4><span><?= Yii::$app->formatter->asDate($post->created_at, 'php: d F Y') ;?></span></h4> <!-- так же можно указать формат даты здесь -->
                    </div>
                </div>
                <?php endforeach;?>
            </div>
